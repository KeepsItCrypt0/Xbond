...
contract xBOND is ERC20, ReentrancyGuard {
    ...
    address[] private _swapPath; // Stored as state variable
    ...
    Reserves private _cachedReserves;
    ...
    constructor(
        ...
        // Initialize swapPath as a dynamic array with length 2
        _swapPath = new address[](2);

        // Set max approvals for xBOND and PLSX tokens using safeIncreaseAllowance
        SafeERC20.safeIncreaseAllowance(IERC20(address(this)), pulseXRouter, type(uint256).max);
        SafeERC20.safeIncreaseAllowance(IERC20(plsx), pulseXRouter, type(uint256).max);
    }
    ...
    function _cacheReserves() internal {
        if (_pairAddress == address(0)) revert ZeroAddress();
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IPulseXPair(_pairAddress).getReserves();
        _cachedReserves = Reserves(reserve0, reserve1, blockTimestampLast);
    }

    function _getReserves() internal view returns (uint112 reserve0, uint112 reserve1) {
        if (_cachedReserves.blockTimestampLast == 0 || _cachedReserves.blockTimestampLast < block.timestamp - _DEADLINE_BUFFER) {
            revert StaleReserves();
        }
        reserve0 = _cachedReserves.reserve0;
        reserve1 = _cachedReserves.reserve1;
    }
    ...
    function _swapTaxToPLSX(uint256 taxAmount) private {
        ...
        // Perform swap (approval already set in constructor)
    ...
    function _createPoolAndAddLiquidity(uint256 plsxAmount, uint256 xBONDAmount) internal {
        ...
        // Set swapPath
        _swapPath[0] = isXBONDToken0 ? address(this) : _plsx;
        _swapPath[1] = isXBONDToken0 ? _plsx : address(this);
        ...
        // Approve LP token for router using safeIncreaseAllowance
        SafeERC20.safeIncreaseAllowance(IERC20(pair), _pulseXRouter, type(uint256).max);

        // Cache reserves after pool creation
        _cacheReserves();
